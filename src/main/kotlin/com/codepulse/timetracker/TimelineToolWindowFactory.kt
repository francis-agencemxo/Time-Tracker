...
import com.codepulse.timetracker.BrowsingTrackerServer
...
class TimelineToolWindowFactory : ToolWindowFactory {
    ...
    override fun createToolWindowContent(project: Project, toolWindow: ToolWindow) {
        ...
        val dashboardButton = JButton("📊").apply {
            toolTipText = "Open Dashboard"
            addActionListener {
                try {
                    val url = java.net.URI("http://localhost:56000/")
                    java.awt.Desktop.getDesktop().browse(url)
                } catch (e: Exception) {
                    JOptionPane.showMessageDialog(null, "Could not open browser: ${e.message}")
                }
            }
        }
