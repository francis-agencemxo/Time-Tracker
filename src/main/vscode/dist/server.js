...
class TrackerServer {
    constructor(database, licenseStore, config) {
    ...
    configureMiddleware() {
        ...
        this.app.use((req, res, next) => {
            ...
            res.header("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept, X-Wrike-Token");
        ...
        if (this.staticRoot && fs.existsSync(this.staticRoot)) {
            this.app.use(express_1.default.static(this.staticRoot));
        }
    }
    configureRoutes() {
        if (!this.app) {
        ...
        router
            ...
            try {
                ...
                await config.update("idleTimeoutMinutes", idleTimeoutMinutes, vscode.ConfigurationTarget.Global);
                ...
                this.config = {
                    ...this.config,
                    idleTimeoutMinutes,
                    storageType: typeof storageType === "string" ? storageType : this.config.storageType ?? "local",
                };
    ...
    durationSeconds(startIso, endIso) {
